...
class UKVFile:
    ...
    def __init__(
        ...
        *,
        ...
        # This path is used for process level locking
    ...
    def write_header(self):
        if self.mode in {"w", "x"}:
            self._stream.seek(0)
            self._pack_write(_FILE_HEADER, self.h1, self.h2)
            self._pack_write(_BLOCK_HEADER, 0, len(self.lb))
            self._stream.write(self.lb)
            self._eof = self._bof
        else:
            raise Exception("Unable to write header unless the file is only being created.")
        
    def read_header(self):
        self._stream.seek(0)
        self.h1, self.h2 = self._unpack_read(_FILE_HEADER)
        klen, vlen = self._unpack_read(_BLOCK_HEADER)
        assert klen == 0, "Leading block must be 0-length"
        self.lb = self._stream.read(vlen)
    
    @property
    def _bof(self):
        return _FILE_HEADER.size + _BLOCK_HEADER.size + len(self.lb)
