...
# this is the actual physical TS, as it was synthesized and confirmed by XRD
...
def pca_score(full_df:pd.DataFrame, dimensions=2, name='test', save_path = None, plot=False, highlight_df: pd.DataFrame=None) -> pd.DataFrame:
...
#    rest = space.loc[[i for i in space.index if i not in sample.index], :]
    ...
    assignments, distances = pairwise_distances_argmin_min(space, sample) # assignments is ndarray of indexes for space, s.t. y[assignments[i]] is closest to sample[i]
...
def kmeans_elbow(df: pd.DataFrame, max_clusters=20, name='test', save_path = None, plot=False) -> tuple:
    ...
    kl = KneeLocator(list(range(1, max_clusters)), distortions, curve='convex', direction='decreasing', online=True)
...
def exemplar_selector(kmeans : KMeans, data: pd.DataFrame) -> list:
    closest, _ = pairwise_distances_argmin_min(kmeans.cluster_centers_, data) # closest is ndarray of indexes s.t. data[closest[i]] is row closest to clusters[i]
...
def tsne_plot(t_df: pd.DataFrame, highlight_df: pd.DataFrame, dimensions: int, name: str, save_path):
        fig = plt.figure()
        # make 2D projection
        ax = fig.add_subplot(111)
        ...
        c = ['red', 'red', 'red', 'red', 'red', 'darkolivegreen', 'darkolivegreen', 'darkolivegreen', 'darkolivegreen', 'darkolivegreen', 'navy', 'navy', 'navy', 'navy', 'navy']

        if highlight_df is None:
            for i in t_df.index.to_list():
                ax.scatter(t_df.loc[i,0],t_df.loc[i,1], alpha = 1, color = c[int(i.split('_')[1]) - 1], label=i)
        else:
            for i in t_df.index.to_list():   # for increasing opacity of specific catalysts (like exemplars)
                if i in highlight_df.index.to_list():
                    ...
                    a = 0.5
                    style = 'o'
                ax.scatter(t_df.loc[i,0],t_df.loc[i,1], alpha = a, marker=style, color = c[int(i.split('_')[1]) - 1], label=i)

        #Can adjust bbox_to_anchor to move legend
        handles, labels = ax.get_legend_handles_labels()
        sorted_labels, sorted_handles = zip(*sorted(zip(labels, handles), key=lambda x: sorting_legend(x[0])))
...
def pca_plot(t_df: pd.DataFrame, pca_test: PCA, highlight_df: pd.DataFrame, dimensions: int, name: str, save_path):
        fig = plt.figure()
