...
def get_oxazaline_alignment_atoms(mol):
        ...
        for atom in nitrogens:
            ...
            for neighbor in neighbors:
                ...
                if str(neighbor.GetHybridization()) == "SP2":
                    ...
                    if sorted([i.GetSymbol() for i in neighbor.GetNeighbors()]) == sorted(
                        ["C", "N", "O"]
                    ):
                        # print('here2')
                        oxazoline_Ns.append(atom)
    ...
    c2 = [
        atom.GetIdx()
        for atom in mol.GetAtomWithIdx(first_N).GetNeighbors()
        if str(atom.GetHybridization()) == "SP2"
    ][0]
    other_c2 = [
        atom.GetIdx()
        for atom in mol.GetAtomWithIdx(second_N).GetNeighbors()
        if str(atom.GetHybridization()) == "SP2"
    ][0]
    # print([i.GetSymbol() for i in mol.GetAtomWithIdx(c2).GetNeighbors()])
    # get bridge
    bridge = set([atom.GetIdx() for atom in mol.GetAtomWithIdx(c2).GetNeighbors()]).intersection(
        set([atom.GetIdx() for atom in mol.GetAtomWithIdx(other_c2).GetNeighbors()])
    )
    assert len(bridge) == 1
    bridge = bridge.pop()
...
def _conf_gen(args) -> ml.ConformerEnsemble:
    ...
    try:  # try to align conformers with reference
        ...
        AllChem.AlignMol(
            mol, reference, atomMap=mol_map
        )  # this line aligns the first conf to the reference mol
        AllChem.AlignMolConformers(
            mol, atomIds=mol_alignment_atoms
        )  # this line aligns mol confs to first conf
    except Exception as exp:
    ...
    if do_calc:
        ...
        sorted_conformers = sorted(
            zip(list(conf_ids), energy_list), key=lambda x: x[1], reverse=False
        )
    ...
    try:  # convert conformers back to mol2 and put into conformer library
        ...
        with progress.get_lock():  # initialize string to hold all conformer mol2 data
            for conf in enumerate(
                filtered_conformers
            ):  # iterate through all filtered conformer ids
            ...
            ensemble = ml.ConformerEnsemble.loads_mol2(
                mol2_string
            )  # create ensemble with string mol2 data
            progress.value += 1
...
def conformer_generation(
    ...
    args = [
        i
        for i in zip(
            [reference_molecule for i in range(length)],
            lib,
            [alignment_atoms for i in range(length)],
            conf_list,
            iter_list,
            thresh_list,
        )
    ...
    try:  # create output file
        ...
        clib = ml.ConformerLibrary.new(file_output, overwrite=overwrite)
    except Exception as exp:
