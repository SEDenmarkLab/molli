...
def visualize_rdkit_mols(name: str, rdkit_mol_list: list, molsPerRow=5, prop:str='_Name', svg=True, png=False):
    '''
    This visualize any RDKit mols that can be kekulized
    '''

    if svg:
        _img = Draw.MolsToGridImage(rdkit_mol_list, molsPerRow=molsPerRow, subImgSize=(200,200), useSVG=True,returnPNG=False, legends=[i.GetProp(prop) for i in rdkit_mol_list], maxMols=1000)
        with open(f'{name}.svg','w') as f:
            f.write(_img.data)
    if png:
        _img = Draw.MolsToGridImage(rdkit_mol_list, molsPerRow=molsPerRow, subImgSize=(200,200), useSVG=False,returnPNG=True, legends=[i.GetProp(prop) for i in rdkit_mol_list], maxMols=1000)
        with open(f'{name}.png','wb') as f:
            f.write(_img.data)

def visualize_molli_mol(name: str, molli_mol_list: list, removeHs=True, molsPerRow=5, svg=True, png=False):
    '''
    This does a basic 2D visualization of the molli_mols with RDKit
    '''
...
def create_rdkit_mol(molli_mol:Molecule, removeHs=False) -> Dict[Molecule,PropertyMol]:
    '''
    ...
    try:
        ...
        rdkit_mol = PropertyMol(Chem.MolFromMol2Block(to_mol2_w_ob(molli_mol), removeHs=removeHs, sanitize=False))
        rdkit_mol.SetProp("_Name", f'{molli_mol.name}')
...
def canonicalize_rdkit_mol(rdkit_mol, sanitize=False) -> PropertyMol:
    ...
    # TODO: Use different function for Property Names as current implementation sometimes ends up empty
...
def reorder_molecule(molli_mol:Molecule, can_rdkit_mol_w_h, can_atom_reorder:list, can_bond_reorder:list):
    '''
    ...
    assert equal_check, f'Array of rdkit atoms: {can_rdkit_atom_elem} is not equal to array of molli atoms: {new_molli_elem}'
